...
(function() {
    ...
    odoo.__DEBUG__ = {
        ...
        getMissingJobs() {
            const waited = new Set(jobs.filter((job)=>!job.ignoreMissingDeps).map((job)=>job.name));
            const missing = new Set();
            for (const job of waited) {
                for (const dep of this.getDependencies(job)) {
                    if (!(dep in this.services) && !waited.has(dep) && !failed.find((job)=>job.name === dep)) {
                        missing.add(dep);
                    }
                }
            }
        ...
        processJobs: function() {
            ...
            function isReady(job) {
                return (!job.error && job.factory.deps.every(function(name) {
    ...
    odoo.log = function() {
        ...
        if (jobs.length) {
            ...
            var unloaded = Object.keys(debugJobs).map(function(key) {
                return debugJobs[key];
            }).filter(function(job) {
                return job.missing;
            });
            if (debug || failed.length || unloaded.length) {
                var log = globalThis.console[!failed.length || !unloaded.length ? "info" : "error"].bind(globalThis.console);
    ...
    function displayModuleErrors({failed, missing, unloaded, cycle}) {
        ...
        if ([failed, missing, unloaded].some((arr)=>arr.length) || cycle) {
            ...
            container.className = "position-fixed w-100 h-100 d-flex align-items-center flex-column bg-white overflow-auto modal";
...
odoo.define('@web/legacy/js/core/cookie_utils', [], function(require) {
    ...
    const utils = {
            ...
            for (var i = 0, l = cookies.length; i < l; i++) {
                ...
                if (cookieName && cookieName === name) {
                    if (cookie.startsWith('"')) {
                        if (cookie.includes('\\')) {
                            throw new Error(`Cookie value contains unknown characters ${cookie}`)
                        }
        ...
        deleteCookie(name) {
            document.cookie = [`${name}=`, 'path=/', `max-age=-1`, `expires=${new Date(new Date().getTime() - 1000).toGMTString()}`, ].join(';');
        },
    };
    __exports[Symbol.for("default")] = utils;
    return __exports;
});
odoo.define(`web.utils.cookies`, ['@web/legacy/js/core/cookie_utils'], function(require) {
    return require('@web/legacy/js/core/cookie_utils')[Symbol.for("default")];
});
;
/* /web/static/src/legacy/js/core/menu.js */
odoo.define('@web/legacy/js/core/menu', [], function(require) {
    'use strict';
    let __exports = {};
    ...
    async function initAutoMoreMenu(el, options) {
        ...
        function _adapt() {
            ...
            _restore();
            if (!el.getClientRects().length || el.closest('.show') || (window.matchMedia(`(max-width: ${options.minSize}px)`).matches && !isNoHamburgerMenu)) {
...
odoo.define(`web.public.lazyloader`, ['@web/legacy/js/public/lazyloader'], function(require) {
    return require('@web/legacy/js/public/lazyloader')[Symbol.for("default")];
});
;
